...
The purpose of this study was to develop pGRF and pLR prediction equations based on accelerometer data able to accurately estimate the mechanical loading during walking and running. The predictions were tested with accelerometers placed at different body locations and using both the resultant vector and its vertical component in a set of incremental speeds, ranging from slow walking to running. Our results demonstrate that accelerometer-based regression models are a valid and accurate method to estimate pGRF, especially with data from hip-worn accelerometers, during human locomotion. The results also indicate that pLR can also be predicted, however with a substantially lower accuracy compared with the pGRF models.

\medskip

All pGRF models showed an accurate prediction, with a MAPE around 5.5\%, using data from hip-worn accelerometers, and around 6.0\% with data from the other placements. On the other hand, the pLR prediction models presented considerably higher errors, as shown by their MAPE around 29\%. Also, differently than the pGRF equations, the pLR equations that showed the best accuracy are from the lower back placement. Moreover, for both outcomes, predictions from the resultant vector tended to be slightly better than from its vertical component. These results are also reflected in the Bland-Altman plots, as the pGRF plots presented considerably narrower limits of agreement and lower dispersion compared to the pLR. Furthermore, it can be observed that the Bland-Altman plot fot the pGRF equation for the resultant vector and data from accelerometers placed at the hip presents a high degree of clustering around the zero and very few data points outside the limits of agreement, which confirms it to be the prediction equation that better performs.

\medskip

In respect to the various accelerometer placements analysed in this study, the hip placement showed the best results, mainly in the pGRF models. This reinforces the hip placement as the established location to collect and analyse accelerometry-measured physical activity data, as this placement is already well accepted for energy expenditure prediction and physical activity intensity classification [@Migueles2017] and has shown a high mechanical loading prediction ability in this and previous studies [@Veras2020; @Neugebauer2018; @Neugebauer2012]. Besides the various placements, the current study also analysed predictions from both the resultant and vertical vectors. Although ACC and GRF from the vertical vector are the most studied and frequently associated with bone health [@Vainionpaa2006; @Heikkinen2007], the resultant vector of both ACC and GRF during locomotion is greatly determined by the vertical vector, and, as can be observed in Supplemental Figure S1, values from both vectors in this study were very similar. Also, mechanical loading prediction based on the resultant ACC offers the important advantage of not being dependent on the accelerometer correct orientation, which is hard to assure when measuring daily physical activity [@Welk2005; @Wundersitz2015].
